...
describe("Token contract", function () {
    ...
    //     await reflectToken.connect(owner).transfer(tokenHolderOne.address, ethers.utils.parseUnits("1.0", 9))
    //     expect(await reflectToken.balanceOf(tokenHolderOne)).to.eq(ethers.utils.parseUnits("0.980000001", 9))
    //     await reflectToken.connect(owner).transfer(tokenHolderTwo.address, ethers.utils.parseUnits("1000.0", 9))

    //     expect(await reflectToken.balanceOf(tokenHolderOne)).to.eq(ethers.utils.parseUnits(".980001961", 9))
    ...
    //     await reflectToken.connect(owner).transfer(tokenHolderOne.address, ethers.utils.parseUnits("100", 9))

    //     await reflectToken.connect(owner).transfer(tokenHolderOne.address, ethers.utils.parseUnits("1000001", 9))
    ...
    //     await reflectToken.connect(owner).transfer(tokenHolderOne.address, ethers.utils.parseUnits("10000", 9))
    //     await reflectToken.connect(owner).transfer(tokenHolderTwo.address, ethers.utils.parseUnits("20000", 9))
    ...
    //     expect(await reflectToken.balanceOf(owner)).to.be.lessThan(ethers.utils.parseUnits(".000001000"));
    // });
    ...
    it("should add to liquidity on transfer", async function () {
        ...
        await reflectToken.connect(owner).transfer(tokenHolderOne.getAddress(), ethers.utils.parseUnits("10000", 9))
        await reflectToken.connect(tokenHolderOne).transfer(tokenHolderTwo.getAddress(), ethers.utils.parseUnits("9999", 9))
        await reflectToken.connect(tokenHolderOne).transfer(tokenHolderTwo.getAddress(), ethers.utils.parseUnits("1", 9))
        console.log("pair balance of owner:", await tokenPair.balanceOf(owner.getAddress()))
...
describe("Liquidation", function () {
  it("should liquidate recipient upon transfer if transfer would increase recipient balance over threshold", async function () {
    ...
    const {reflectToken, tokenPair} = await addInitialLiquidity(owner);
    await reflectToken.connect(owner).enableLiquidation()
    await reflectToken.connect(owner).transfer(tokenHolderOne.getAddress(), ethers.utils.parseUnits("69420133780081.3", 9))
    expect(await reflectToken.balanceOf(tokenHolderOne.getAddress())).to.eq(ethers.utils.parseUnits("69420133780081.3", 9))
    //this transaction puts it at exactly 1% of total supply
    await reflectToken.connect(owner).transfer(tokenHolderOne.getAddress(), ethers.utils.parseUnits(".05", 9))
    expect(await reflectToken.balanceOf(tokenHolderOne.getAddress())).to.eq(ethers.utils.parseUnits("0.0", 9))
  });
  it("should liquidate sender upon transfer if sender's balance is over threshold at time of transfer", async function () {
    ...
    // await reflectToken.connect(owner).transfer(tokenHolderTwo.getAddress(), ethers.utils.parseUnits("69420133780081.35", 9))
    ...
    // await reflectToken.connect(tokenHolderOne).transfer(tokenHolderThree.getAddress(), ethers.utils.parseUnits("1", 9));
...
async function addInitialLiquidity(owner) {
  ...
  const wethAddress = await weth.address;
  
  const ReflectToken = await ethers.getContractFactory("REFLECT");
  const reflectToken = await ReflectToken.deploy(owner.getAddress(), routerAddress);
  const reflectTokenAddress = await reflectToken.address;
  ...
  await reflectToken.connect(owner).approve(routerAddress, ethers.utils.parseUnits("10000000000000", 18))
  await router
  .connect(owner)
  .addLiquidity(
    reflectTokenAddress,
